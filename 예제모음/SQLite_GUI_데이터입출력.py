...
def insertData() :
    ...
    sql="CREATE TABLE userTable(id char(4),etc1 varchar(10), etc2 varchar(10), birth int)"

    con = sqlite3.connect("C:/sqlite/sonDB")  # DB가 저장된 폴더까지 지정
    cur = con.cursor()

    #cur.execute(sql) #첫실행때 한번만 재생
...
def updateData():
    ...
    try :
        sql = "UPDATE userTable SET id='" + data5 + "'WHERE id='" + data1 + "'"
...
btnUpdate = Button(edtFrame, text = "업데이트", command = updateData)
